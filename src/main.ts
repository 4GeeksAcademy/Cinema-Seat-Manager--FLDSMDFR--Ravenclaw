...
function buscarAsientosContiguos(sala: number[][]):  {primero: { fila: number; columna: number }; segundo: { fila: number; columna: number } } | null {
        for (let fila = 0; fila < sala.length; fila++) {
...
function crearSalaLlena(): number[][] {
    const sala = getSala();
...
function mostrarResumenEscenario(nombre: string, sala: number[][]): void {
    console.log(`\n--- ${nombre} ---`);
    imprimirSala(sala);
    ...
    if (contiguos) {
        console.log(
            `Primer par contiguo libre: F${contiguos.primero.fila + 1}, C${contiguos.primero.columna + 1} y C${contiguos.segundo.columna + 1}.`
        );
    } else {
...
function ejecutarPruebasEscenarios(): void {
    // 1) Sala vacía.
...
if (asientosContiguos) {
    console.log(
        `Primer par de asientos contiguos libres encontrado en F${asientosContiguos.primero.fila + 1}, C${asientosContiguos.primero.columna + 1} y C${asientosContiguos.segundo.columna + 1}.`
    );
} else {
    console.log("No hay pares de asientos contiguos disponibles en la sala.");
}
